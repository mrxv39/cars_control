...
export function VDVehicleDocs({ docs, vehicleId, onReload }: { docs: api.VehicleDocument[]; vehicleId: number; onReload: () => void }) {
  ...
  const completedCount = DOC_TYPES.filter(({ key }) => docs.find((d) => d.doc_type === key)).length;
  ...
  return (
    <section className="panel vd-sidebar-panel">
      ...
      <div className="vd-doc-completion">
        <span className="vd-doc-completion-text">{completedCount}/{totalCount} documentos</span>
        <div className="vd-doc-completion-bar">
          <div className="vd-doc-completion-fill" style={{ width: `${(completedCount / totalCount) * 100}%` }} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        {DOC_TYPES.map(({ key, label }) => {
          ...
          return (
            <div key={key} className="vd-doc-row">
              <div style={{ display: "flex", alignItems: "center", gap: "0.55rem", fontSize: "var(--text-sm)" }}>
                <span
                  role="img"
                  aria-label={doc ? "Adjuntado" : "Pendiente"}
                  style={{ width: 8, height: 8, borderRadius: "50%", background: doc ? "var(--color-success)" : "var(--color-text-faint)", flexShrink: 0 }}
                />
                <span className="vd-doc-icon">{DOC_ICONS[key]}</span>
